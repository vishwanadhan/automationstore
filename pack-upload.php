...
$userType = $loginObj->fetchUserType();	  
	...
	{
		/* echo"<script>window.location.href='pack-list.php'</script>";
        exit; */
	}
...
?>
	...
	<section class="upload-form">
		<form enctype="multipart/form-data" method="post" action="workflow-pack-upload-edit.php" name="upload_form" id="upload_form">
			...
			<ul>		
				<li>
					<ul class="mtop">
						...
						<ul>
							<li class="ralign">
							<!-- <input type="image" src="images/cancel-btn.png" name="cancel"> -->
							<input type="button" class="myButton" name="cancel" value="Cancel" onClick="javascript:window.location.replace('pack-list.shtml');" />   
			...
			?>	
				...
				<?php
					$sql = "SELECT uuid, packName, version, certifiedBy, packFilePath  FROM ".TBL_PACKDETAILS." where post_approved='true'";  		
					...
					if($num > 0){
						...
						while($line = $packObj->getResultObject($rst)) {      
							...
							$genTable .= '<a href="workflow-pack-upload-confirm.shtml?uuid='.$line->uuid.'&version='.$line->version.'">'.wrapText($line->packName,40).'</a>'; 
							$genTable .= "</td>";
