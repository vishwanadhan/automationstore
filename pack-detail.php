...
$searchText = (isset($_POST['search']) ? $_POST['search'] : null);
		...
		/* Start block -> Dev ASG */
			/*  $packUuid = (isset($_POST["packUuid"]) ? $_POST["packUuid"] : null);	
			 $packVersion = (isset($_POST["packVersion"]) ? $_POST["packVersion"] : null);
			 
			// if($packUuid != null && $packVersion != null){
			//	$packIds = $entityObj->getPackId($packUuid, $packVersion);					
			// } */
		/* End block -> Dev ASG */	
...
function setDeprecationFlag(id,version) {
        ...
        $.ajax({
            url:"pass.php",
            data:{action:"setDeprecationFlag", packtype:"Workflow", packId: id, packVesion:version, status:checkValue},
            type:"POST",
            success:function(response)
            {
			   //alert(response);
...
<div id="popup1" class="modal-box">
  ...
  <p>Select reasons(s) for flagging:</p>
   ...
   <input type="hidden" name="detailPageUrl" value="<?php echo "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']; ?>" />
    <input type="hidden" name="packType" id="packType" value="Workflow" />
...
<div id="popup2" class="modal-box">
	...
	<form name="packDeprecation" id="packDeprecation" action="" method="post" onSubmit="return submitDepForm();">
		...
		<input type="hidden" name="depPackType" id="depPackType" value="Workflow" />
