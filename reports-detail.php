...
function setDeprecationFlag(id,version) {
        ...
        $.ajax({
            url:"pass.php",
            data:{action:"setDeprecationFlag", packtype:"Report", packId: id, packVesion:version, status:checkValue},
            type:"POST",
...
<body>
    ...
    ?>
	<?php
	/***** Grievance Reporting for Reports Start *****/
	if($_POST['submitFlag'] == 'Flag'){
		$responseReport =  @$packobj->addGrievance($_POST); 
		$_POST = '';
	}
	/***** Grievance Reporting for Reports End *****/
	
	/***** Star rating for Reports Start *****/
	if(isset($_POST['submit_rating'])){
		//print_r($_POST); exit;
		$responseRating =  @$packobj->addStarRating($_POST); 
		$_POST = '';
	}
	/***** Star rating for Reports End *****/
    ...
    <div id="body_content">
        ...
        <form name="entityForm" id="entityForm" method="post" action="">
            <?php
            $reportVersion = (isset($_REQUEST["reportVersion"]) ? $_REQUEST["reportVersion"] : null);
			$reportName = (isset($_REQUEST["reportName"]) ? $_REQUEST["reportName"] : null);
			
			if ($reportName == null || $reportVersion == null) {
               header('Location: reports.php');
            } else {
               echo @$entityObj->reportInformation($reportName,$reportVersion,"");
           }
            ?> 	
        </form>
    </div>
...
<?php
$reportnameget = implode(" ",explode("!",$reportName));
?>
  ...
  <p>Select reasons(s) for flagging:</p>
    ...
    <p>Comment <span class="red">*</span><textarea name="flagComment" class="inputbox" style="resize:none;" placeholder="Leave Comment" maxlength="1000"></textarea>
   ...
   <input type="hidden" name="detailPageUrl" value="<?php echo "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']; ?>" />
    <input type="hidden" name="packType" id="packType" value="OCUM" />   
    <input type="hidden" name="packVersion" id="packVersion" value="<?php echo $reportVersion; ?>" />
    <input type="hidden" name="packName" id="packName" value="<?php echo $reportnameget; ?>" />
    <input type="hidden" name="packContactEmail" id="packContactEmail" class="inputbox" value="<?php echo ($packContactEmail = $entityObj->getReportContactEmail($reportnameget, $reportVersion)); ?>" />
  </div>
   <footer> 
...
<div id="popup2" class="modal-box">
	<header> <a href="javascript:void(0)" class="js-modal-close close"><img src="images/close-icon.png" border="0" /></a>
		<h2>Deprecate Report</h2>
	</header>
	<form name="packDeprecation" id="packDeprecation" action="" method="post">
		<div class="modal-body">
		<p>Your comments will be stored and others can view it.</p>
			
		<p>Comment <span class="red">*</span><textarea name="depComment" class="inputbox" style="resize:none;" placeholder="Leave reason for requesting to deprecate the report" autofocus="autofocus"></textarea>
		<div class="popup-error" id="depComment_error"></div></p>
		
		<input type="hidden" name="detailPageUrl" value="<?php echo "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI']; ?>" />
		<input type="hidden" name="depPackid" id="depPackid" value="<?php echo $packid = $entityObj->getReportId($reportnameget, $reportVersion); ?>" />
		<input type="hidden" name="depPackType" id="depPackType" value="Report" />
		<input type="hidden" name="depPackVersion" id="depPackVersion" value="<?php echo $reportVersion; ?>" />
